...
<body class="bg-[#f8fafc] text-slate-800 font-sans antialiased">
        ...
        <div class="max-w-7xl mx-auto px-6 h-16 flex justify-between items-center">
            ...
            <div class="flex items-center gap-4">
                <div class="hidden md:flex flex-col items-end mr-2">
                    ...
                    <span class="text-sm font-bold text-slate-700"><?php echo e(auth()->user()->name); ?></span>
                </div>
    ...
    <main class="max-w-7xl mx-auto px-6 py-10">
        <div class="mb-12">
            ...
            <?php $__empty_1 = true; $__currentLoopData = $pinjamanAktif; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $aktif): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <?php
                ...
                <div
                    ...
                    <div class="flex items-center gap-4">
                        <div
                            class="w-10 h-10 rounded-xl flex items-center justify-center <?php echo e($isTelat ? 'bg-rose-100 text-rose-600' : ($isPending ? 'bg-amber-100 text-amber-600' : 'bg-indigo-100 text-indigo-600')); ?>">
                            <i class="fa-solid <?php echo e($isPending ? 'fa-clock' : 'fa-toolbox'); ?>"></i>
                        </div>
                        <div>
                            <h3 class="font-bold text-slate-800 text-sm"><?php echo e($aktif->alat->nama_alat); ?></h3>
                            <p class="text-[11px] font-medium text-slate-500">
                                <?php if($isPending): ?>
                                    Menunggu verifikasi petugas
                                <?php else: ?>
                                    Batas: <?php echo e(\Carbon\Carbon::parse($aktif->tgl_kembali)->format('H:i')); ?>

                                    <?php echo $isTelat ? '<span class="text-rose-500 font-bold ml-1">(Terlambat!)</span>' : ''; ?>
                    ...
                    <div class="text-right">
                        <span
                            class="text-[10px] uppercase font-black tracking-widest <?php echo e($isTelat ? 'text-rose-600' : ($isPending ? 'text-amber-500' : 'text-indigo-600')); ?>">
                            <?php echo e($isPending ? 'Pending' : ($isTelat ? 'Denda' : 'Dipinjam')); ?>
        ...
        <div class="mb-12">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
                <?php $__empty_1 = true; $__currentLoopData = $alats; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <div
                        ...
                        <div class="p-5">
                            <span
                                class="text-[10px] font-black text-slate-400 uppercase tracking-widest"><?php echo e($item->kode_alat); ?></span>
                            <h3 class="font-black text-lg text-slate-800 mt-1"><?php echo e($item->nama_alat); ?></h3>
                            <p class="text-sm text-slate-500 mt-2 line-clamp-2"><?php echo e($item->spesifikasi); ?></p>

                            <a href="<?php echo e(route('peminjam.create', ['alat_id' => $item->id])); ?>"
            ...
            <div class="bg-white border border-slate-200 rounded-[32px] overflow-hidden shadow-sm">
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        <?php $__empty_1 = true; $__currentLoopData = $riwayatSelesai; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $history): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <tr class="hover:bg-slate-50/50 transition-colors">
                                <td class="px-6 py-4 font-bold text-slate-700 text-sm">
                                    <?php echo e($history->alat->nama_alat); ?>

                                </td>

                                <td class="px-6 py-4 text-center text-xs text-slate-500">
                                    <?php echo e(\Carbon\Carbon::parse($history->tgl_pinjam)->setTimezone('Asia/Makassar')->format('d M Y, H:i')); ?>

                                </td>
                                <td class="px-6 py-4 text-center text-xs text-slate-500">
                                    <?php echo e(\Carbon\Carbon::parse($history->tgl_kembali)->setTimezone('Asia/Makassar')->format('d M Y, H:i')); ?>

                                </td>       
                                <td class="px-6 py-4 text-center text-xs font-bold bg-indigo-50/10">
                                    <?php if($history->status == 'ditolak'): ?>
                                        <span class="text-slate-300">-</span>
                                    <?php elseif($history->tgl_kembali_asli): ?>
                                        <span class="text-indigo-600">
                                            <?php echo e(\Carbon\Carbon::parse($history->tgl_kembali_asli)->setTimezone('Asia/Makassar')->format('d M Y, H:i')); ?>
                                ...
                                <td
                                    class="px-6 py-4 text-right text-xs font-bold <?php echo e($history->denda > 0 ? 'text-rose-600' : 'text-slate-400'); ?>">
                                    <?php echo e($history->denda > 0 ? 'Rp ' . number_format($history->denda) : '-'); ?>
    ...
    <?php if(session('success')): ?>
        <script>
            Swal.fire({
                ...
                html: '<span class="text-slate-500 font-medium"><?php echo e(session('success')); ?></span>',
...
<?php /**PATH D:\xampp\htdocs\peminjaman_alat\resources\views/peminjam/index.blade.php ENDPATH**/ ?>