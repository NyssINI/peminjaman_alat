...
<?php $__env->startSection('content'); ?>
    <div class="max-w-7xl mx-auto px-6 py-10">
        ...
        <div class="bg-white border border-slate-200 rounded-[24px] p-6 mb-8 shadow-sm">
            <form action="<?php echo e(route('datapeminjaman.index')); ?>" method="GET" class="flex flex-wrap items-end gap-4">
                ...
                <div class="w-40">
                    ...
                    <select name="status"
                        ...
                        <option value="disetujui" <?php echo e(request('status') == 'disetujui' ? 'selected' : ''); ?>>Disetujui
                        </option>
        ...
        <div class="bg-white border border-slate-200 rounded-[32px] overflow-hidden shadow-sm">
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-slate-100">
                    <?php $__empty_1 = true; $__currentLoopData = $datapeminjaman; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="hover:bg-slate-50/50 transition-colors">
                            <td class="px-6 py-4">
                                <div class="font-bold text-slate-800 text-sm"><?php echo e($item->user->name); ?></div>
                                <div class="text-[11px] text-slate-500 font-medium italic"><?php echo e($item->alat->nama_alat); ?>

                                    (<?php echo e($item->alat->kode_alat); ?>)
                                </div>
                            </td>
                            <td class="px-6 py-4 text-center text-xs text-slate-500 font-medium">
                                <?php echo e(\Carbon\Carbon::parse($item->tgl_pinjam)->format('d M Y, H:i')); ?>

                            </td>
                            <td class="px-6 py-4 text-center text-xs text-slate-500 font-medium">
                                <?php echo e(\Carbon\Carbon::parse($item->tgl_kembali)->format('d M Y, H:i')); ?>

                            </td>
                            <td class="px-6 py-4 text-center text-xs text-slate-500 font-medium">
                                <?php echo e(\Carbon\Carbon::parse($item->tgl_asli)->format('d M Y, H:i')); ?>
                            ...
                            <td class="px-6 py-4 text-center">
                                ...
                                ?>
                                <span
                                    class="px-3 py-1 <?php echo e($class); ?> text-[10px] font-black rounded-lg border uppercase tracking-widest">
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex justify-end gap-2">
                                    <button type="button"
                                        onclick="openModal(<?php echo e(json_encode($item->load('user', 'alat'))); ?>)"
                                    ...
                                    <form action="<?php echo e(route('datapeminjaman.destroy', $item->id)); ?>" method="POST"
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\xampp\htdocs\peminjaman_alat\resources\views/datapeminjaman/index.blade.php ENDPATH**/ ?>